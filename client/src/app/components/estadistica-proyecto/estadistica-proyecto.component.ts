...
export class EstadisticaProyectoComponent implements OnInit {
  ...
  usuarios: any = [];
  displayedColumns: string[] = ['Actividad', 'Fecha de inicio', 'Fecha de finalización', 'Fecha de término', 'Empleado que elabora', 'Acciones'];
  ...
  constructor(private actividadesService: ActividadService, private usuarioService: UsuarioService, private toastr: ToastrService, private proyectoService: ProyectoService, private activatedRoute: ActivatedRoute, private router: Router){}
  ...
  ngOnInit() {
    ...
    forkJoin([
      ...
      ([completas, pendientes, retrasadas, proyecto]) => {
        ...
        this.porcentajeProyecto = (100 * this.completas.cantidadResultados) / this.total;
        this.dibujarGrafica();
  ...
  obtenerCompletas(){
    this.actividadesService.actividadesCumplidasProyecto(this.proyectoId).subscribe(res => {
      this.completas = res;
  ...
  obtenerPendientes(){
    this.actividadesService.actividadesPendientesProyecto(this.proyectoId).subscribe(res => {
      this.pendientes = res;
  ...
  obtenerRetrasadas(){
    this.actividadesService.actividadesRetrasadasProyecto(this.proyectoId).subscribe(res => {
      this.retrasadas = res;
